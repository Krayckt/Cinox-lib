...
function Cinox.Init(manifest)
	...
	elseif manifest.Background_Mode == "C" and manifest["C/A-B_Color"] then 
		bgColor = typeof(manifest["C/A-B_Color"]) == "Color3" and manifest["C/A-B_Color"] or Color3.fromHex(tostring(manifest["C/A-B_Color"]))
	end
	...
	TitleText.BackgroundTransparency = 1; TitleText.TextColor3 = Color3.new(1,1,1)
	TitleText.Font = Enum.Font.GothamBold; TitleText.TextXAlignment = "Left"; TitleText.Parent = TitleBar
	...
	if manifest.USER_INFO then
		...
		UserInfo.BackgroundTransparency = 1; UserInfo.TextColor3 = Color3.new(1,1,1); UserInfo.TextXAlignment = "Left"; UserInfo.Parent = MainFrame
	end
	...
	UserInputService.InputBegan:Connect(function(input, gpe)
		if not gpe and input.KeyCode == Enum.KeyCode[manifest.ToggelKeybind or "RightShift"] then ScreenGui.Enabled = not ScreenGui.Enabled end
	end)
	...
	function Lib:AddTab(config)
		...
		if layoutType == "TABEL" then
			local grid = Instance.new("UIGridLayout", Page); grid.CellSize = isMobile and UDim2.new(0, 100, 0, 100) or UDim2.new(0, 130, 0, 130)
		else Instance.new("UIListLayout", Page).Padding = UDim.new(0, 5) end
